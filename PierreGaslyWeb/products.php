...
$inactive_brands = $db->fetchAll("SELECT * FROM brands WHERE is_active = 0 ORDER BY brand_name");
$inactive_sizes = $db->fetchAll("SELECT * FROM product_sizes WHERE is_active = 0 ORDER BY size_kg");

function redirectProductsPage(string $message = '', string $type = 'success'): void {
    $params = ['view' => $_GET['view'] ?? 'active'];
    if ($message !== '') {
        $params[$type] = $message;
    }
...
// Handle Remove Brand
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_brand'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        redirectProductsPage('Invalid security token', 'error');
    }

    $brand_id = (int)($_POST['brand_id'] ?? 0);
    $brand = $db->fetchOne("SELECT * FROM brands WHERE brand_id = ?", [$brand_id]);

    if (!$brand) {
        redirectProductsPage('Brand not found', 'error');
    }

    $inUse = $db->fetchOne("SELECT COUNT(*) as count FROM products WHERE brand_id = ? AND is_active = 1", [$brand_id]);
    if ((int)($inUse['count'] ?? 0) > 0) {
        redirectProductsPage('Cannot remove brand while it is used by active products', 'error');
    }

    if ($db->query("UPDATE brands SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE brand_id = ?", [0, $brand_id])) {
        logActivity('update', 'brand', $brand_id, "Removed brand: " . $brand['brand_name']);
        redirectProductsPage("Brand '" . $brand['brand_name'] . "' removed successfully!");
    }

    redirectProductsPage('Failed to remove brand. ' . ($db->getLastError() ?: ''), 'error');
}

// Handle Restore Brand
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['restore_brand'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        redirectProductsPage('Invalid security token', 'error');
    }
    ...
    if ($db->query("UPDATE brands SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE brand_id = ?", [1, $brand_id])) {
        logActivity('update', 'brand', $brand_id, "Restored brand: " . $brand['brand_name']);
        redirectProductsPage("Brand '" . $brand['brand_name'] . "' restored successfully!");
    }

    redirectProductsPage('Failed to restore brand. ' . ($db->getLastError() ?: ''), 'error');
}

// Handle Remove Size
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_size'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        redirectProductsPage('Invalid security token', 'error');
    }

    $size_id = (int)($_POST['size_id'] ?? 0);
    $size = $db->fetchOne("SELECT * FROM product_sizes WHERE size_id = ?", [$size_id]);

    if (!$size) {
        redirectProductsPage('Size not found', 'error');
    }

    $inUse = $db->fetchOne("SELECT COUNT(*) as count FROM products WHERE size_kg = ? AND is_active = 1", [(int)$size['size_kg']]);
    if ((int)($inUse['count'] ?? 0) > 0) {
        redirectProductsPage('Cannot remove size while it is used by active products', 'error');
    }

    if ($db->query("UPDATE product_sizes SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE size_id = ?", [0, $size_id])) {
        logActivity('update', 'size', $size_id, "Removed size: " . (int)$size['size_kg'] . 'kg');
        redirectProductsPage('Size ' . (int)$size['size_kg'] . 'kg removed successfully!');
    }

    redirectProductsPage('Failed to remove size. ' . ($db->getLastError() ?: ''), 'error');
}

// Handle Restore Size
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['restore_size'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        redirectProductsPage('Invalid security token', 'error');
    }
    ...
    if ($db->query("UPDATE product_sizes SET is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE size_id = ?", [1, $size_id])) {
        logActivity('update', 'size', $size_id, "Restored size: " . (int)$size['size_kg'] . 'kg');
        redirectProductsPage('Size ' . (int)$size['size_kg'] . 'kg restored successfully!');
    }

    redirectProductsPage('Failed to restore size. ' . ($db->getLastError() ?: ''), 'error');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_brand'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        ...
        $existing = $db->fetchOne("SELECT * FROM brands WHERE brand_name = ?", [$brand_name]);
        
        if ($existing && (int)($existing['is_active'] ?? 0) === 1) {
            $error = 'Brand already exists';
        } elseif ($existing) {
            if ($db->query("UPDATE brands SET is_active = ? WHERE brand_id = ?", [1, $existing['brand_id']])) {
                $success = "Brand '$brand_name' restored successfully!";
                logActivity('update', 'brand', (int)$existing['brand_id'], "Restored brand: $brand_name");
                $brands = $db->fetchAll("SELECT * FROM brands WHERE is_active = 1 ORDER BY brand_name");
            } else {
                $error = 'Failed to restore brand. ' . ($db->getLastError() ?: '');
            }
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_size'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        ...
        $allowed_sizes = [7, 11, 22, 50];
        
        if (!in_array($size_kg, $allowed_sizes, true)) {
            $error = 'Please select a valid LPG size';
        } else {
            $existing = $db->fetchOne("SELECT * FROM product_sizes WHERE size_kg = ?", [$size_kg]);
            
            if ($existing && (int)($existing['is_active'] ?? 0) === 1) {
                $error = 'Size already exists';
            } elseif ($existing) {
                if ($db->query("UPDATE product_sizes SET is_active = ? WHERE size_id = ?", [1, $existing['size_id']])) {
                    $success = "Size {$size_kg}kg restored successfully!";
                    logActivity('update', 'size', (int)$existing['size_id'], "Restored size: {$size_kg}kg");
                    $sizes = $db->fetchAll("SELECT * FROM product_sizes WHERE is_active = 1 ORDER BY size_kg");
                } else {
                    $error = 'Failed to restore size. ' . ($db->getLastError() ?: '');
                }
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_product'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        ...
        $removeCurrentImage = !empty($_POST['remove_current_image']);
        $product_image = $removeCurrentImage ? null : $existingImage;
        ...
        if (isset($_FILES['product_image']) && $_FILES['product_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($upload['success']) {
                ...
                $oldImageName = basename($existingImage);
                $oldImagePath = UPLOAD_PATH . 'products/' . $oldImageName;
                if ($oldImageName !== '' && is_file($oldImagePath)) {
                    @unlink($oldImagePath);
                }
            } else {
                $error = $upload['message'];
            }
        } elseif ($removeCurrentImage && $existingImage !== '') {
            $oldImageName = basename($existingImage);
            $oldImagePath = UPLOAD_PATH . 'products/' . $oldImageName;
...
<div class="page-header">
    <div class="header-kicker">Catalog</div><h1>Products Management</h1>
...
<div class="quick-actions-section">
    ...
    <div class="action-buttons">
        ...
        <button onclick="document.getElementById('brandManageModal').classList.add('active')" class="btn-outline">
            Manage Brands
        </button>

        <button onclick="document.getElementById('sizeManageModal').classList.add('active')" class="btn-outline">
            Manage Sizes
        </button>
    </div>
...
<div id="productModal" class="modal <?php echo $editProduct ? 'active' : ''; ?>">
    <div class="modal-content" style="max-width: 700px; max-height: 90vh; overflow-y: auto;">
        <button class="btn-close-improved" onclick="closeProductModal()" title="Close">
        ...
        <div class="modal-body">
            <form method="POST" action="" enctype="multipart/form-data" id="productForm">
                ...
                <?php if ($editProduct): ?>
                    ...
                    <input type="hidden" name="existing_image" value="<?php echo htmlspecialchars($editProduct['product_image'] ?? ''); ?>">
                <?php endif; ?>
                    ...
                    <div class="form-group">
                        ...
                        <?php if ($editProduct && !empty($editProduct['product_image'])): ?>
                            <div style="margin-top: 12px; display:flex; gap:14px; align-items:center; flex-wrap:wrap;">
                                <img src="<?php echo htmlspecialchars((preg_match('/^https?:\/\//i', $editProduct['product_image']) ? $editProduct['product_image'] : BASE_URL . ltrim(preg_replace('#^(?:/?uploads/products/)+#i', 'uploads/products/', $editProduct['product_image']), '/'))); ?>" alt="Current product image" style="width:72px;height:72px;object-fit:cover;border-radius:14px;border:1px solid #dbe4ef;">
                                <div>
                                    <small style="color: #666; display:block; margin-bottom:8px;">Current: <?php echo htmlspecialchars($editProduct['product_image']); ?></small>
                                    <label style="display:inline-flex; align-items:center; gap:8px; font-size:13px; color:#334155;">
                                        <input type="checkbox" name="remove_current_image" value="1">
...
<div id="brandManageModal" class="modal">
    <div class="modal-content" style="max-width: 560px;">
        <div class="modal-header">
            <span class="btn-close" onclick="closeModal('brandManageModal')">&times;</span>
            <h3>Manage Brands</h3>
        </div>
        <div class="modal-body">
            <p style="margin-bottom: 16px; color: #718096;">Choose one active brand to remove. Removed brands stay hidden here and can be added back later using the Add Brand action.</p>
            <form method="POST" style="display: grid; gap: 12px;">
                <input type="hidden" name="csrf_token" value="<?php echo $csrfToken; ?>">
                <label style="font-weight: 600;">Remove active brand</label>
                <select class="form-control" name="brand_id" <?php echo empty($brands) ? 'disabled' : ''; ?> required>
                    <?php if (empty($brands)): ?>
                        <option value="">No active brands</option>
                    <?php else: ?>
                        <?php foreach ($brands as $brand): ?>
                            <option value="<?php echo (int)$brand['brand_id']; ?>"><?php echo htmlspecialchars($brand['brand_name']); ?></option>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </select>
                <button type="submit" name="remove_brand" class="btn btn-danger" <?php echo empty($brands) ? 'disabled' : ''; ?>>Remove Brand</button>
            </form>
            <div style="display:flex; justify-content:flex-end; margin-top:16px;">
                <button type="button" onclick="closeModal('brandManageModal')" class="btn btn-cancel">Close</button>
            </div>
        </div>
    </div>
</div>
...
<!-- Manage Sizes Modal -->
<div id="sizeManageModal" class="modal">
    <div class="modal-content" style="max-width: 560px;">
        <div class="modal-header">
            <span class="btn-close" onclick="closeModal('sizeManageModal')">&times;</span>
            <h3>Manage Sizes</h3>
        </div>
        <div class="modal-body">
            <p style="margin-bottom: 16px; color: #718096;">Choose one active LPG size to remove. Removed sizes stay hidden here and can be added back later using the Add Size action.</p>
            <form method="POST" style="display: grid; gap: 12px;">
                <input type="hidden" name="csrf_token" value="<?php echo $csrfToken; ?>">
                <label style="font-weight: 600;">Remove active LPG size</label>
                <select class="form-control" name="size_id" <?php echo empty($sizes) ? 'disabled' : ''; ?> required>
                    <?php if (empty($sizes)): ?>
                        <option value="">No active sizes</option>
                    <?php else: ?>
                        <?php foreach ($sizes as $size): ?>
                            <option value="<?php echo (int)$size['size_id']; ?>"><?php echo (int)$size['size_kg']; ?> kg</option>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </select>
                <button type="submit" name="remove_size" class="btn btn-danger" <?php echo empty($sizes) ? 'disabled' : ''; ?>>Remove Size</button>
            </form>
            <div style="display:flex; justify-content:flex-end; margin-top:16px;">
                <button type="button" onclick="closeModal('sizeManageModal')" class="btn btn-cancel">Close</button>
            </div>
        </div>
    </div>
</div>
