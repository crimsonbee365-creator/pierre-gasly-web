...
try {
    ...
    $users = $supabase->select('users', ['email' => $email], '*', true);
    
    if (empty($users)) {
        logError('Verify OTP: user row not found after successful auth verify for ' . $email);
        sendError('Your email was verified, but your profile row could not be loaded. Please try logging in.', 500);
    }
