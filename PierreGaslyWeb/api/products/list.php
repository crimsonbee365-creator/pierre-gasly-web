...
function pg_app_url(): string
{
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $detected = $scheme . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost');
    $env = getenv('APP_URL') ?: $detected;
    return rtrim($env, '/');
}

function build_image_url(?string $rawPath): ?string
{
    ...
    // Normalize slash direction and strip accidental duplicate prefixes.
    $path = str_replace('\\', '/', $path);
    $path = preg_replace('#^(?:\./)+#', '', $path);
    $path = preg_replace('#^(?:/?uploads/products/)+#i', 'uploads/products/', ltrim($path, '/'));
    $path = preg_replace('#^(?:/?uploads/)+#i', 'uploads/', ltrim($path, '/'));

    if (stripos($path, 'uploads/') === 0) {
        return $base . '/' . str_replace(' ', '%20', $path);
    }

    return $base . '/uploads/products/' . rawurlencode(basename($path));
}
...
try {
    ...
    foreach ($rows as &$row) {
        ...
        $row['image_url'] = build_image_url($image);
    }
    unset($row);
