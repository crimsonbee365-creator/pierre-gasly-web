...
function showMessage(input, message, isError = false) {
    // Try to anchor messages within the nearest form-group to avoid duplicates
    const group = input.closest('.form-group') || input.parentElement;

    // If multiple hints exist (from previous scripts/rerenders), keep only one
    const existingHints = group ? group.querySelectorAll('.input-hint') : [];
    if (existingHints && existingHints.length > 1) {
        for (let i = 1; i < existingHints.length; i++) {
            existingHints[i].remove();
        }
    }

    let hint = group ? group.querySelector('.input-hint') : null;

    if (!hint) {
        hint = document.createElement('div');
        hint.className = 'input-hint';

        // Insert after password wrapper if it exists, otherwise after the input
...
// Helper: skip inputs inside forms marked data-no-validate
function isExcluded(input) {
    return !!input.closest('form[data-no-validate]');
}
