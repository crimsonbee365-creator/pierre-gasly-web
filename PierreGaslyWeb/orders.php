...
function normalizeFulfillmentMethod($order) {
    $fulfillment = strtolower(trim((string)($order['fulfillment_method'] ?? '')));
    if (in_array($fulfillment, ['pickup', 'pick-up', 'branch pickup'], true)) {
        return 'pickup';
    }
    if (in_array($fulfillment, ['cod', 'delivery', 'cash on delivery'], true)) {
        return 'cod';
    }
    ...
    if (strpos($address, 'branch pickup') !== false || strpos($address, 'pickup schedule') !== false || strpos($address, 'pickup at') !== false) {
        return 'pickup';
    }
