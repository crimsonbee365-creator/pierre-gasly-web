...
class Database {
    ...
    public function fetchAll($query, $params = []) {
        ...
        // Simple SELECT * FROM table with optional WHERE/ORDER BY support
        if (preg_match('/SELECT \* FROM (\w+)(?: WHERE (.*?))?(?: ORDER BY .*)?$/', trim($query), $matches)) {
            $table = $matches[1];
            $filters = [];
            if (!empty($matches[2])) {
                $whereClause = trim($matches[2]);

                if (preg_match('/^(\w+)\s*=\s*(?:1|true)$/i', $whereClause, $whereMatches)) {
                    $filters[$whereMatches[1]] = 1;
                } elseif (preg_match('/^(\w+)\s*=\s*(?:0|false)$/i', $whereClause, $whereMatches)) {
                    $filters[$whereMatches[1]] = 0;
                } elseif (preg_match('/^(\w+)\s*=\s*\?$/', $whereClause, $whereMatches) && isset($params[0])) {
                    $filters[$whereMatches[1]] = $params[0];
                }
            }
