...
function upsertSystemSetting($db, $key, $value, $userId = null) {
    $existing = $db->select('system_settings', ['setting_key' => $key]);
    $payload = ['setting_value' => (string)$value];
    if ($userId !== null) {
        $payload['updated_by'] = $userId;
    }

    if (!empty($existing)) {
        return $db->update('system_settings', $payload, ['setting_key' => $key]) !== null;
    }

    $payload['setting_key'] = $key;
    return $db->insert('system_settings', $payload) !== null;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['download_backup'])) {
    if (!isMasterAdmin()) {
        ...
        $tables = ['users', 'products', 'orders', 'sales', 'activity_logs', 'user_rewards', 'system_settings', 'reviews'];
        $snapshot = [
            'generated_at' => date('c'),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_settings'])) {
    if (!verifyCSRFToken($_POST['csrf_token'])) {
        ...
        $openingTime = sanitize($_POST['opening_time'] ?? '08:00');
        $closingTime = sanitize($_POST['closing_time'] ?? '18:00');
        $businessHours = date('g:i A', strtotime($openingTime)) . ' - ' . date('g:i A', strtotime($closingTime));

        $settings = [
            'system_name' => sanitize($_POST['system_name']),
            'delivery_fee_tier_1' => (float)$_POST['delivery_fee_tier_1'],
            'delivery_fee_tier_2' => (float)$_POST['delivery_fee_tier_2'],
            'delivery_fee_tier_3' => (float)$_POST['delivery_fee_tier_3'],
            'delivery_fee' => (float)$_POST['delivery_fee_tier_1'],
            'branch_city' => 'Dagupan City',
            'service_province' => 'Pangasinan',
            'pickup_enabled' => isset($_POST['pickup_enabled']) ? '1' : '0',
            'cod_enabled' => isset($_POST['cod_enabled']) ? '1' : '0',
            'opening_time' => $openingTime,
            'closing_time' => $closingTime,
            'pickup_after_hours_rule' => 'Pickup requests placed after closing time are processed the next day',
        ...
        foreach ($settings as $key => $value) {
            if (upsertSystemSetting($db, $key, $value, $_SESSION['user_id'])) {
                $updated++;
            }
        }
...
$activities = $db->fetchAll(
    "SELECT a.*, COALESCE(u.full_name, 'System') as full_name 
...
<div class="settings-container">
    <div class="page-header">
        <div class="header-kicker">Configuration</div><h1>System Settings</h1>
        ...
        <div class="settings-card">
            ...
            <div class="settings-card-body">
                <form method="POST" action="">
                    ...
                    <div class="form-group full-width">
                        <label>Delivery Fee by Distance Tier</label>
                        <div class="form-grid">
                            <div class="form-group">
                                <label>Tier 1 Fee (₱)</label>
                                <div class="input-group">
                                    <span class="input-prefix">₱</span>
                                    <input type="number" name="delivery_fee_tier_1" step="0.01" required class="form-control has-prefix" 
                                           value="<?php echo settingValue($settings, 'delivery_fee_tier_1', settingValue($settings, 'delivery_fee', '50.00')); ?>">
                                </div>
                                <small style="color: #666; font-size: 12px;">Nearest delivery areas from Dagupan City</small>
                            </div>

                            <div class="form-group">
                                <label>Tier 2 Fee (₱)</label>
                                <div class="input-group">
                                    <span class="input-prefix">₱</span>
                                    <input type="number" name="delivery_fee_tier_2" step="0.01" required class="form-control has-prefix" 
                                           value="<?php echo settingValue($settings, 'delivery_fee_tier_2', '90.00'); ?>">
                                </div>
                                <small style="color: #666; font-size: 12px;">Mid-distance delivery areas within Pangasinan</small>
                            </div>

                            <div class="form-group">
                                <label>Tier 3 Fee (₱)</label>
                                <div class="input-group">
                                    <span class="input-prefix">₱</span>
                                    <input type="number" name="delivery_fee_tier_3" step="0.01" required class="form-control has-prefix" 
                                           value="<?php echo settingValue($settings, 'delivery_fee_tier_3', '130.00'); ?>">
                                </div>
                                <small style="color: #666; font-size: 12px;">Farthest delivery areas within Pangasinan only</small>
                            </div>

                            <div class="form-group">
                                <label>Service Coverage</label>
                                <input type="text" class="form-control" value="Pangasinan only" readonly>
                                <small style="color: #666; font-size: 12px;">Orders outside Pangasinan should not be accepted</small>
                            </div>
                        </div>
                    </div>

                    <div class="form-group full-width">
                        <label>Order Fulfillment Setup</label>
                        <div class="form-grid">
                            <div class="form-group">
                                <label style="margin-bottom: 12px;">Available Methods</label>
                                <label style="display:flex; align-items:center; gap:10px; font-weight:500; margin-bottom:10px;">
                                    <input type="checkbox" name="cod_enabled" value="1" <?php echo settingValue($settings, 'cod_enabled', '1') === '1' ? 'checked' : ''; ?>>
                                    Cash on Delivery
                                </label>
                                <label style="display:flex; align-items:center; gap:10px; font-weight:500;">
                                    <input type="checkbox" name="pickup_enabled" value="1" <?php echo settingValue($settings, 'pickup_enabled', '1') === '1' ? 'checked' : ''; ?>>
                                    Branch Pickup
                                </label>
                                <small style="color: #666; font-size: 12px; display:block; margin-top:8px;">Customers can either choose COD delivery or pickup at the Dagupan City branch.</small>
                            </div>

                            <div class="form-group">
                                <label>Opening Time</label>
                                <input type="time" name="opening_time" required class="form-control"
                                       value="<?php echo htmlspecialchars(settingValue($settings, 'opening_time', '08:00')); ?>">
                                <small style="color: #666; font-size: 12px;">Default: 8:00 AM</small>
                            </div>

                            <div class="form-group">
                                <label>Closing Time</label>
                                <input type="time" name="closing_time" required class="form-control"
                                       value="<?php echo htmlspecialchars(settingValue($settings, 'closing_time', '18:00')); ?>">
                                <small style="color: #666; font-size: 12px;">Default: 6:00 PM</small>
                            </div>

                            <div class="form-group">
                                <label>After-Hours Pickup Rule</label>
                                <input type="text" class="form-control" readonly
                                       value="Orders placed after store hours will be processed tomorrow.">
                                <small style="color: #666; font-size: 12px;">Applied to branch pickup scheduling in checkout</small>
                            </div>
                        </div>
                    </div>
